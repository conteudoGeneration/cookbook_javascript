...
**Bancos de dados do tipo Coluna** utilizam **colunas** para armazenar dados, em vez de linhas, como nos **bancos de dados relacionais**. Embora o conceito de banco de dados de colunas possa parecer similar ao modelo relacional à primeira vista, o verdadeiro poder desse tipo de banco de dados reside na sua abordagem **não normalizada** para a organização e estruturação dos dados.  Na imagem abaixo, vemos a estrutura básica de uma coluna:

<div align="center"><img src="https://i.imgur.com/EkTUiXI.png" title="source: imgur.com"/></div>
...
Para uma melhor compreensão da estrutura de dados em um banco de dados de colunas, é fundamental conhecer e entender os conceitos: **Keyspace**, **Famílias de Colunas** e **Chave Primária**.

- **Keyspace**: É um **container** ou **namespace** onde as tabelas (famílias de colunas) são armazenadas. Em termos simples, é equivalente a um banco de dados em sistemas relacionais tradicionais.
- **Família de Colunas (Column Family)**: É uma coleção de colunas que pertencem a uma tabela no banco de dados de colunas. Cada família de colunas é composta por várias colunas e organizada por um **identificador de linha** (ou chave primária), o que permite agrupar os dados de forma eficiente.
...
- **Chave Primária**: Assim como nos bancos de dados relacionais, a chave primária em um banco de dados de colunas é usada para identificar de forma única as linhas dentro da **Família de Colunas**. Isso é fundamental para o particionamento e distribuição dos dados no cluster, garantindo que os dados sejam armazenados e acessados de maneira eficiente.

> Em um contexto de bancos de dados distribuídos, um **cluster** é um conjunto de **nós** (servidores) que trabalham em conjunto para fornecer um serviço de banco de dados escalável, resiliente e distribuído. Nesse ambiente, os dados são distribuídos e replicados entre os nós, permitindo que o sistema lide com grandes volumes de dados, mantendo alta disponibilidade e desempenho aprimorado.

A chave primária em uma família de colunas pode ser composta apenas pelo **identificador único** (como um ID) ou por uma combinação de campos (por exemplo, ID + data de criação), onde o segundo campo define a ordenação dos dados dentro da partição.

Na imagem abaixo, vemos uma representação gráfica de uma **Família de Colunas de Usuários**, onde a **chave primária** é composta apenas pelo **id** (identificador único):
...
Os **bancos de dados de colunas** são otimizados para a **recuperação rápida de dados**, o que os torna ideais para **aplicativos analíticos** e **consultas de leitura intensiva**. O armazenamento orientado a colunas melhora a performance de consultas analíticas, pois reduz significativamente os requisitos de **E/S de disco** e diminui a quantidade de dados que precisam ser carregados do disco.

Alguns exemplos de bancos de dados baseados em colunas incluem **Amazon DynamoDB**, **Cassandra** e **Google Cloud Datastore**.
